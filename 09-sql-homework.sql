...
FROM
    ...
    city ct ON ct.city_id = a.city_id
        INNER JOIN
    country cr ON cr.country_id = ct.country_id
WHERE
    cr.country = 'canada';
...
SELECT 
    SUM(p.amount) AS 'Total Amount', i.store_id
FROM
    payment p
        INNER JOIN
...
SELECT 
    s.store_id, ct.city, cr.country
FROM
    country cr
        INNER JOIN
    city ct ON ct.country_id = cr.country_id
        INNER JOIN
